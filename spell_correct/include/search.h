...
namespace WORD_CORRECT
{
    ...
    class CWord_correct
    {
        ...
        public:
            ...
            void make_cache(std::set<std::pair<std::string,int> >& cache,strhash& r_index,const std::string& search_word);
            void cache_out(std::set<std::pair<std::string,int> >& cache);
            std::string operator()(const std::string& search_word);
    };
}
#endif
